...
import { readNumberCurveKeys, sampleNumberCurve, type NumberCurveInterpolation } from './numberCurve.ts';
...
const curveNode = definition({
  ...
  createConfig: () => ({ keys: [{ id: 'key-0', time: 0, value: 0, interpolation: 'bezier', tangentMode: 'auto' }, { id: 'key-1', time: 1, value: 1, interpolation: 'bezier', tangentMode: 'auto' }] }),
  evaluate: (_context, inputs, config) => {
    const keys = readNumberCurveKeys(config.keys);
    return { value: sampleNumberCurve(keys, number(inputs.time), String(config.interpolation ?? 'linear') as NumberCurveInterpolation | 'smoothstep') };
  },
});
