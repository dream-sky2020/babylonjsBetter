...
import { DAMAGE_TAGS, SKILL_EFFECTS, STATUS_TYPES, TRIGGER_TIMINGS } from '../config.ts';
import { useCombatStore } from '../store.ts';

interface SkillEffectItemProps {
  side: 'left' | 'right';
  unitId: string;
  skillId: string;
  effectIndex: number;
...
export const SkillEffectItem: React.FC<SkillEffectItemProps> = ({ side, unitId, skillId, effectIndex }) => {
  const data = useCombatStore(state => {
    ...
    return skill?.skillEffects[effectIndex];
  });

  const updateSkillEffect = useCombatStore(state => state.updateSkillEffect);
  const removeSkillEffect = useCombatStore(state => state.removeSkillEffect);

  if (!data) return null;

  const onPatch = (updates: Record<string, unknown>) => {
    updateSkillEffect(side, unitId, skillId, effectIndex, updates);
  };
  ...
  return (
    <EffectItemContainer
      ...
      timing={data.timing}
      type={data.type}
      onTimingChange={(timing) => onPatch({ timing })}
      onTypeChange={(type) => {
        if (type === 'applyStatus') {
        ...
        onPatch({ type, value: 0 });
      }}
      onRemove={() => removeSkillEffect(side, unitId, skillId, effectIndex)}
    >
      {data.type === 'dmg' ? (
        <DamageEffectItem
          value={data.value || 0}
...
const EffectItemContainer: React.FC<EffectItemContainerProps> = ({
  withTiming = false,
  timing,
  type,
  children,
...
interface HealEffectItemProps {
  value: number;
  onChange: (updates: Record<string, unknown>) => void;
}

const HealEffectItem: React.FC<HealEffectItemProps> = ({ value, onChange }) => {
  return (
    <>
      <span>数值</span>
      <input
