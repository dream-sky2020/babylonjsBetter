...
import type { NormalizedUv, SpriteAnchorPreset } from '../types/sprite-anchors.types';
import { createIconPlane, type SpriteFrameRegion } from './meshFactory';
export type { SpriteFrameRegion } from './meshFactory';

export type SpritePresetSource = 'merged' | 'config' | 'local';

export type SpritePresetProvider = {
  toSpritePresetKey: (texturePath: string, frameName?: string) => string;
  getSpriteAnchorPreset: (
    texturePathOrPresetKey: string,
    source?: SpritePresetSource,
    frameName?: string
  ) => SpriteAnchorPreset;
};
...
export const createMockSprite = (
  ...
  presetProvider: SpritePresetProvider
): MockSprite => {
  const iconPlane = createIconPlane(scene, texturePath, baseSize);
  const mesh = iconPlane.mesh;
  iconPlane.setFrameRegion(frameRegion);
  let currentPreset = presetProvider.getSpriteAnchorPreset(texturePath, presetSource);

  return {
    mesh,
    texturePath: presetProvider.toSpritePresetKey(texturePath),
    ...
    refreshPreset() {
      currentPreset = presetProvider.getSpriteAnchorPreset(texturePath, presetSource);
