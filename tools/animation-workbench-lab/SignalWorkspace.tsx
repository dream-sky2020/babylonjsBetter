...
import { setContributionArmed, type ContributionOperation, type ContributionRecordProperty, type TransformRecordMode } from './transformRecording.ts';
...
type Props = Readonly<{
  ...
  onRecordContributionProperty(bindingId: string, property: Exclude<ContributionRecordProperty, 'value'>, value: number): void;
  onBeginEdit(): void;
  onEndEdit(): void;
...
export function SignalWorkspace({ graph, bindings, objects, selectedObjectId, transport, events, time, recordMode, recordOperation, onTimeChange, onRecordModeChange, onRecordOperationChange, onRecordContributionProperty, onBeginEdit, onEndEdit, onGraphChange, onBindingsChange, onTransportChange, onEventsChange, onEvaluate }: Props) {
  ...
  const deleteNode = (id: string) => {
    ...
    if (removedOutputIds.size) onBindingsChange(bindings.filter(binding => !removedOutputIds.has(binding.outputId)).map(binding => ({ ...binding, config: Object.fromEntries(Object.entries(binding.config).map(([key, value]) => key.endsWith('OutputId') && removedOutputIds.has(String(value)) ? [key, ''] : [key, value])) })));
  ...
  const removeOutput = (outputId: string) => {
    ...
    onBindingsChange(bindings.filter(binding => binding.outputId !== outputId).map(binding => ({ ...binding, config: Object.fromEntries(Object.entries(binding.config).map(([key, value]) => key.endsWith('OutputId') && value === outputId ? [key, ''] : [key, value])) })));
  };
  ...
  const addBinding = () => {
    ...
    onBindingsChange([...bindings, { id: makeId('binding'), outputId: output.id, objectId: object.id, adapterTypeId: 'babylon.transform-component.number', config: { path: 'position.y', operation: recordOperation, scale: 1, offset: 0, weight: 1, modulation: 1, priority: 0, enabled: true, solo: false, modulationOutputId: '', recordArmed: false, recordProperty: 'value' } }]); setTab('output');
  };
  ...
  return <section className="awb-signal-dock">
    <header>
      ...
      <div className="awb-transport">
        <div className="awb-record-modes" aria-label="关键帧录制模式"><button className={recordMode === 'off' ? 'active' : ''} onClick={() => onRecordModeChange('off')} title="编辑基础值，不自动创建关键帧">EDIT</button><button className={recordMode === 'auto' ? 'active auto' : ''} onClick={() => onRecordModeChange('auto')} title="仅对已 Arm 的现有动画属性自动成键">AUTO</button><button className={recordMode === 'record' ? 'active record' : ''} onClick={() => onRecordModeChange('record')} title="写入已 Arm 的 Contribution；无来源时自动创建"><i />REC</button></div>
        <label className="awb-record-source"><span>NEW</span><select value={recordOperation} onChange={event => onRecordOperationChange(event.target.value as ContributionOperation)}><option value="additive">Add</option><option value="override">Override</option><option value="multiply">Multiply</option></select></label>
    ...
    {tab === 'output' && <div className="awb-output-page"><section><header><div><strong>LIVE OUTPUTS</strong><span>公开信号保持独立，不预先烘焙为固定动作。</span></div></header>{graph.outputs.map(output => <div className="awb-output-row" key={output.id}><i /><input value={output.name} onChange={event => onGraphChange({ ...graph, outputs: graph.outputs.map(item => item.id === output.id ? { ...item, name: event.target.value } : item) })} /><code>{output.valueTypeId}</code><b>{formatValue(evaluation.outputs.get(output.id) ?? null)}</b><button onClick={() => removeOutput(output.id)}><MiniIcon name="delete" /></button></div>)}{!graph.outputs.length && <div className="awb-empty">先在 Graph 中公开一个节点输出</div>}<header className="awb-mix-result-title"><div><strong>MIXED TARGETS</strong><span>每个目标每帧只接收一个最终值。</span></div></header>{[...contributionMix.values].map(([target, value]) => <div className="awb-mix-result" key={target}><span>{target.replace(':', ' · ')}</span><b>{value.toFixed(4)}</b><small>{contributionMix.groups.get(target)?.filter(item => item.effectiveWeight > 0).length ?? 0} active</small></div>)}</section><section><header><div><strong>CONTRIBUTION MIXER</strong><span>Arm 后 REC 只写入这个目标上的指定来源与属性。</span></div><button onClick={addBinding} disabled={!graph.outputs.length}><MiniIcon name="add" />添加 {recordOperation === 'additive' ? 'Add' : recordOperation === 'override' ? 'Override' : 'Multiply'}</button></header>{bindings.map(binding => {
      const path = String(binding.config.path ?? 'position.y'); const targetKey = previewTargetKey(binding.objectId, path); const applied = contributionMix.groups.get(targetKey)?.find(item => item.id === binding.id);
      const armed = binding.config.recordArmed === true; const recordProperty = String(binding.config.recordProperty ?? 'value') as ContributionRecordProperty;
      const recordNumber = (property: Exclude<ContributionRecordProperty, 'value'>, value: number) => onRecordContributionProperty(binding.id, property, value);
      return <article className={`awb-contribution-card ${binding.config.enabled === false ? 'muted' : ''} ${binding.config.solo === true ? 'solo' : ''} ${armed ? 'armed' : ''}`} key={binding.id}>
        <div className="awb-contribution-main"><button className={`awb-toggle ${binding.config.enabled === false ? 'active' : ''}`} title="启用/静音" onClick={() => updateBindingConfig(binding, { enabled: binding.config.enabled === false })}>M</button><button className={`awb-toggle ${binding.config.solo === true ? 'active' : ''}`} title="仅播放同目标上的 Solo 贡献" onClick={() => updateBindingConfig(binding, { solo: binding.config.solo !== true })}>S</button><button className={`awb-arm ${armed ? 'active' : ''}`} title="Arm/Disarm 录制来源；同一目标只允许一个" onClick={() => onBindingsChange(setContributionArmed(bindings, binding.id, !armed))}>{armed ? 'ARM' : 'A'}</button><select value={binding.outputId} onChange={event => updateBinding(binding.id, { outputId: event.target.value })}>{graph.outputs.map(output => <option key={output.id} value={output.id}>{output.name}</option>)}</select><span>→</span><select value={binding.objectId} onChange={event => updateBinding(binding.id, { objectId: event.target.value, config: { ...binding.config, recordArmed: false } })}>{objects.map(object => <option key={object.id} value={object.id}>{object.name}</option>)}</select><select value={path} onChange={event => updateBindingConfig(binding, { path: event.target.value, recordArmed: false })}>{['position.x','position.y','position.z','rotation.x','rotation.y','rotation.z','scaling.x','scaling.y','scaling.z'].map(item => <option key={item}>{item}</option>)}</select><button className="awb-contribution-delete" onClick={() => onBindingsChange(bindings.filter(item => item.id !== binding.id))}><MiniIcon name="delete" /></button></div>
        <div className="awb-contribution-record"><label>RECORD TARGET<select value={recordProperty} onChange={event => updateBindingConfig(binding, { recordProperty: event.target.value })}><option value="value">Source Value</option><option value="weight">Weight</option><option value="scale">Scale</option><option value="offset">Offset</option><option value="modulation">Modulation</option></select></label><small>{armed ? `${recordMode.toUpperCase()} → ${recordProperty}` : 'Disarmed'}</small></div>
        <div className="awb-contribution-controls"><label>BLEND<select value={String(binding.config.operation ?? 'additive')} onChange={event => updateBindingConfig(binding, { operation: event.target.value })}><option value="additive">Add</option><option value="override">Override</option><option value="multiply">Multiply</option></select></label><label className={binding.config.weightOutputId ? 'animated' : ''}>WEIGHT<input type="number" min="0" step="0.05" value={finite(binding.config.weight, 1)} onChange={event => recordNumber('weight', Number(event.target.value))} /></label><label>PRIORITY<input type="number" step="1" value={finite(binding.config.priority)} onChange={event => updateBindingConfig(binding, { priority: Number(event.target.value) })} /></label><label className={binding.config.scaleOutputId ? 'animated' : ''}>SCALE<input type="number" step="0.1" value={finite(binding.config.scale, 1)} onChange={event => recordNumber('scale', Number(event.target.value))} /></label><label className={binding.config.offsetOutputId ? 'animated' : ''}>OFFSET<input type="number" step="0.1" value={finite(binding.config.offset, 0)} onChange={event => recordNumber('offset', Number(event.target.value))} /></label><label>MOD SIGNAL<select value={String(binding.config.modulationOutputId ?? '')} onChange={event => updateBindingConfig(binding, { modulationOutputId: event.target.value })}><option value="">None</option>{graph.outputs.filter(output => output.valueTypeId === 'core.number').map(output => <option key={output.id} value={output.id}>{output.name}</option>)}</select></label><label className={binding.config.modulationValueOutputId ? 'animated' : ''}>MOD VALUE<input type="number" step="0.05" value={finite(binding.config.modulation, 1)} onChange={event => recordNumber('modulation', Number(event.target.value))} /></label><output className={applied?.suppressed ? 'suppressed' : ''}>{applied?.suppressed ? 'SOLO 抑制' : `W ${applied?.effectiveWeight.toFixed(2) ?? '0.00'} · ${applied?.valueAfter.toFixed(3) ?? '—'}`}</output></div>
      </article>;
    })}{!bindings.length && <div className="awb-empty">添加 Contribution，Arm 后即可安全录制指定来源</div>}</section></div>}
  </section>;
}
