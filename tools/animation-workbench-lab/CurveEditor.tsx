...
import { curveSelectionId, moveCurveKeys, panCurveView, pasteCurveKeys, selectCurveKeysInRect, transformCurveKeys, zoomCurveView, type CurveBatchTransform, type CurveClipboardKey, type CurveKeySelection } from './curveEditorModel.ts';
...
export function CurveEditor({ graph, bindings, objects, selectedObjectId, selectedNodeId, time, duration, onTimeChange, onSelectedNodeIdChange, onGraphChange, onCreateCurve, onBeginEdit, onEndEdit }: Props) {
  ...
  const applyBatch = (transform: CurveBatchTransform) => {
    if (!selection.length) return;
    const changed = transformCurveKeys(keysByCurve, selection, transform, duration);
    onGraphChange({ ...graph, nodes: graph.nodes.map(node => changed.has(node.id) ? { ...node, config: { ...node.config, keys: changed.get(node.id) } } : node) });
  };
  const fitAll = () => {
  ...
  return <div className="awb-curve-layout">
    ...
    <aside className="awb-key-inspector"><header><strong>KEY INSPECTOR</strong><small>{selection.length} selected</small></header>{activeKey && activeSelection ? <><div className="awb-curve-key-form"><label><span>TIME</span><input type="number" step="0.001" value={activeKey.time} onChange={event => updateSelectedKey({ time: clamp(Number(event.target.value), 0, duration) })} /></label><label><span>VALUE</span><input type="number" step="0.01" value={activeKey.value} onChange={event => updateSelectedKey({ value: Number(event.target.value) })} /></label><label><span>SEGMENT</span><select value={activeKey.interpolation ?? 'linear'} onChange={event => setInterpolation(event.target.value as NumberCurveInterpolation)}><option value="constant">Constant</option><option value="linear">Linear</option><option value="smooth">Smooth</option><option value="bezier">Bezier</option></select></label>{activeKey.interpolation === 'bezier' && <><label><span>TANGENTS</span><select value={activeKey.tangentMode ?? 'auto'} onChange={event => setTangentMode(event.target.value as NumberCurveTangentMode)}><option value="auto">Auto</option><option value="free">Free</option><option value="broken">Broken</option></select></label><label><span>IN SLOPE</span><input disabled={(activeKey.tangentMode ?? 'auto') === 'auto'} type="number" step="0.1" value={activeKey.inTangent ?? 0} onChange={event => { const value = Number(event.target.value); updateSelectedKey(activeKey.tangentMode === 'free' ? { inTangent: value, outTangent: value } : { inTangent: value }); }} /></label><label><span>OUT SLOPE</span><input disabled={(activeKey.tangentMode ?? 'auto') === 'auto'} type="number" step="0.1" value={activeKey.outTangent ?? 0} onChange={event => { const value = Number(event.target.value); updateSelectedKey(activeKey.tangentMode === 'free' ? { inTangent: value, outTangent: value } : { outTangent: value }); }} /></label></>}</div><div className="awb-curve-batch"><header><strong>BATCH TRANSFORM</strong></header><label><span>SET VALUE</span><input type="number" step="0.01" value={batchValue} onChange={event => setBatchValue(Number(event.target.value))} /><button onClick={() => applyBatch({ setValue: batchValue })}>Apply</button></label><label><span>TIME OFFSET</span><input type="number" step="0.01" value={timeOffset} onChange={event => setTimeOffset(Number(event.target.value))} /></label><label><span>VALUE OFFSET</span><input type="number" step="0.01" value={valueOffset} onChange={event => setValueOffset(Number(event.target.value))} /></label><label><span>TIME SCALE</span><input type="number" step="0.1" value={timeScale} onChange={event => setTimeScale(Number(event.target.value))} /></label><label><span>VALUE SCALE</span><input type="number" step="0.1" value={valueScale} onChange={event => setValueScale(Number(event.target.value))} /></label><div className="awb-curve-batch-actions"><button onClick={() => applyBatch({ timeOffset, valueOffset, timeScale, valueScale })}>Transform</button><button onClick={() => applyBatch({ timeScale: -1 })}>Mirror Time</button><button onClick={() => applyBatch({ valueScale: -1 })}>Mirror Value</button></div></div></> : <div className="awb-track-empty">选择关键帧后可编辑时间、数值、插值和切线。</div>}</aside>
  </div>;
}
