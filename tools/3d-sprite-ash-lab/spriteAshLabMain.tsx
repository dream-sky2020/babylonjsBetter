...
import{createSpriteAshEffect,normalizeSpriteAshPreset,normalizeSpriteAshPresetLibrary,SPRITE_ASH_PARAMETER_DEFINITIONS,type SpriteAshEffectController,type SpriteAshPreset,type SpriteAshPresetLibrary}from'@/core/sprite';
...
const EFFECT_MODE_OPTIONS=[
 {value:'ash',label:'灰烬燃烧'},
 {value:'blackShards',label:'黑色碎片坠落'},
 {value:'embers',label:'余烬爆散'},
 {value:'frost',label:'冰晶侵蚀'},
 {value:'pixel',label:'像素解体'},
 {value:'void',label:'虚空吞噬'}
] as const;
...
const App:React.FC=()=>{
 ...
 return <div className="lab"><aside className="panel"><h1>3D 精灵化灰实验室</h1><div className="subtle">独立精灵 Shader 调试，不加载任何怪物模块。</div>
  ...
  <section className="section"><div className="section-head"><strong>预设</strong><button onClick={addPreset} disabled={!activePreset}>新增</button></div><label>消散预设</label><select value={activePreset?.presetKey||''} onChange={event=>setPresetKey(event.target.value)}>{Object.values(presets).map(item=><option key={item.presetKey} value={item.presetKey}>{item.name} · {item.presetKey}</option>)}</select>{activePreset&&<><label>消散算法</label><select value={activePreset.effectMode} onChange={event=>updatePreset({...activePreset,effectMode:event.target.value as SpriteAshPreset['effectMode']})}>{EFFECT_MODE_OPTIONS.map(item=><option key={item.value} value={item.value}>{item.label}</option>)}</select><div className="effect-note">{activePreset.effectMode==='blackShards'?'分块变黑后受重力坠落；“上浮距离”会作为下落强度。':'切换算法后，下面的参数仍可继续微调。'}</div><label>预设名称</label><input type="text" value={activePreset.name} onChange={event=>updatePreset({...activePreset,name:event.target.value})}/><div className="grid2 section"><button onClick={duplicate}>复制</button><button onClick={remove} disabled={Object.keys(presets).length<=1}>删除</button></div></>}</section>
 ...
 </aside><main className="stage"><canvas ref={canvasRef}/><a className="top-link" href="/">返回调试入口</a><div className="stage-hint">拖动旋转 3D 视角 · 滚轮缩放 · 可手动拖动化灰进度</div></main></div>
};
