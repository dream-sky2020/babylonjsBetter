...
export const dungeonGridDebugLabModule: LabModule = {
  ...
  setup(context) {
    ...
    const render = () => {
      ...
      root = new TransformNode(`dungeon_grid_debug_${session.sessionId}`, context.scene);
      const material = new StandardMaterial(`dungeon_grid_debug_material_${session.sessionId}`, context.scene);
      material.diffuseColor = Color3.FromHexString('#36bff2');
      material.emissiveColor = Color3.FromHexString('#17698a');
      material.alpha = 0.2;
      material.wireframe = true;
      session.preset.map.tiles.forEach((tile) => {
        const layout = resolveDungeonMapTileWorldLayout(
          session.binding.component,
          session.preset.map.width,
          session.preset.map.height,
          tile.x,
          tile.y,
        );
        const box = MeshBuilder.CreateBox(`dungeon_grid_debug_${session.sessionId}_${tile.x}_${tile.y}`, {
