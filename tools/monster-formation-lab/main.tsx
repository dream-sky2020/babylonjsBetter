...
const normalizeBattlefield=(value:Partial<Battlefield>,fallbackId:string):Battlefield=>({
 ...
 monsters:Array.isArray(value.monsters)?value.monsters.map((monster,index)=>({
  ...
  monsterConfigKey:typeof monster.monsterConfigKey==='string'?monster.monsterConfigKey:'',monsterStripePresetKey:typeof monster.monsterStripePresetKey==='string'?monster.monsterStripePresetKey:'',
  chaos:monster.chaos??{value:0,threshold:100,duration:0},
  position:{
   row:indexInt(monster.position?.row??monster.row),column:indexInt(monster.position?.column??monster.column,index),size:positiveInt(monster.position?.size??monster.slots),
...
const App:React.FC=()=>{
 const canvasRef=useRef<HTMLCanvasElement>(null),stageRef=useRef<HTMLElement>(null),visualManagerRef=useRef<MonsterVisualManager|null>(null);
 ...
 useEffect(()=>{
  ...
  const context=createCameraLabScene(canvas);context.camera.target.set(0,-1,-8);context.camera.radius=34;
  const camera=createCameraLabController(context.camera),panel=createFloatingCameraControlPanel(stage,camera);visualManagerRef.current=new MonsterVisualManager(context.scene);
  ...
  return()=>{canvas.removeEventListener('pointerdown',pointerDown);canvas.removeEventListener('pointermove',pointerMove);canvas.removeEventListener('pointerup',pointerEnd);canvas.removeEventListener('pointercancel',pointerEnd);canvas.removeEventListener('wheel',wheel);document.removeEventListener('mousemove',lockedMove);document.removeEventListener('pointerlockchange',lockChange);window.removeEventListener('keydown',keyDown);window.removeEventListener('keyup',keyUp);window.removeEventListener('resize',resize);visualManagerRef.current?.dispose();visualManagerRef.current=null;panel.dispose();context.dispose()};
 },[]);
 useEffect(()=>{
  if(!battlefield)return;
  visualManagerRef.current?.sync(battlefield,{configs,monsterStripes,stripes},selectedId);
 },[battlefield,configs,monsterStripes,stripes,selectedId]);
