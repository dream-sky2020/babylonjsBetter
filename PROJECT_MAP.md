...
`core/ui/DungeonMapCanvas.tsx` 将地图静态绘制与拖拽框交互分层：指针移动只修改拖拽引用，并经 `requestAnimationFrame` 合并更新轻量 DOM 覆盖框，不再触发 React 更新、整图重绘或分配第二张全尺寸高 DPI Canvas；松开后仍按原规则计算选择并重绘已提交的选择结果。
